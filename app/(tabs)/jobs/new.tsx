...
export default function NewJobScreen() {
  ...
  const onSubmit = async () => {
    if (!business || !session || !customerId || !title.trim()) return;
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.form}>
        ...
        <TextInput
          style={styles.input}
          placeholder="e.g. Weekly mow, Spring cleanup"
          value={title}
        ...
        <Text style={styles.label}>Estimated price (optional)</Text>
        <TextInput
          style={styles.input}
          placeholder="150.00"
        ...
        <Pressable
          style={[styles.button, (submitting || !customerId || !title.trim()) && styles.buttonDisabled]}
          onPress={onSubmit}
          disabled={submitting || !customerId || !title.trim()}
        >
          {submitting ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Create job</Text>}
        </Pressable>
      </View>
    </ScrollView>
  );
